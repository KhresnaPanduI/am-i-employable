...
import { analyzeJobFit, analyzeJobFitFromPdf } from "@/lib/analysis";
...
export async function POST(request: Request) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    if (typeof jobDescription !== "string") {
      return NextResponse.json({ error: "Paste the job description for OCR fallback." }, { status: 400 });
    }

    try {
      const result = await analyzeJobFitFromPdf({
